...
if (isset($_REQUEST['record']) && $_REQUEST['record'] != '') {
	$focus->id = $record;
	$focus->mode = 'edit';
	$log->debug("Mode is Edit. MassiveMovements is ".$focus->id);
	$focus->retrieve_entity_info($record, 'MassiveMovements');
...
if ($focus->mode == 'edit') {
	...
	$associated_prod = getAssociatedProducts('MassiveMovements', $focus);
...
if (isset($_REQUEST['return_module'])) {
	...
	$smarty->assign("RETURN_MODULE", "MassiveMovements");
}
...
//if create MassiveMovements, get all available product taxes and shipping & Handling taxes
if ($focus->mode != 'edit') {
...
if ($focus->mode == 'edit') {
	$inventory_cur_info = getInventoryCurrencyInfo('MassiveMovements', $focus->id);
